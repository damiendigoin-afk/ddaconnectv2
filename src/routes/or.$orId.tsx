    ...
    <AppShell title={formatPlate(v?.plate ?? "")} subtitle={interventionLabel(order.data as { or_number?: string | null; internal_ref?: string | null } | undefined)} back={{ to: "/atelier" }}>
      {order.isLoading ? (
        ...
        <div className="space-y-4">
          ...
          {order.data?.or_number && (order.data as unknown as { site_id?: string | null }).site_id ? (
            <Link to="/pieces-achats/controle-winmotor" search={{ site: (order.data as unknown as { site_id: string }).site_id, or: order.data.or_number }} className="block rounded-xl border-2 border-border bg-card px-4 py-3 text-sm font-extrabold uppercase">
